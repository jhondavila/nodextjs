...
Ext.Inventory.prototype = {
    ...
    getNamesByExpression: function (expression, exclude, accumulate) {
        ...
        for (i = 0; i < length; ++i) {
            if ((expr = expressions[i]).indexOf('*') < 0) {
                ...
                var pathFolder = me.getPathFolder(subExp), singleName, searchExt, searchName, fileInRoute;
                require("fs").readdirSync(pathFolder).forEach(function (file) {
                    searchExt = file.search(/\.js/g);
                    if (searchExt > 0) {
                        searchName = file.replace(/\.js/g, "");
                        fileInRoute = subExp + "." + searchName;
                        nameToAliases[fileInRoute] = [];
                    }
                });
    ...
    getPathFolder: function (className) {
        var me = this,
            paths = me.paths,
            ret = '',
            prefix;
